...
import { importRouterModule, importTranslateModule } from '../../utils/ast-utils';
...
export async function generateFrontendFeatureLibrary(tree: Tree, normalizedOptions: NormalizedSchema): Promise<GeneratorCallback> {
	...
	if (moduleSource) {
		let newModuleSourceFile = importRouterModule(moduleSource, [
			{
				path: '',
		...
		newModuleSourceFile = importTranslateModule(newModuleSourceFile);
